...
The Android client compiles through [`Vaguei.Maui.slnx`](Vaguei.Maui.slnx). It uses exclusive preparation, curriculum analysis, loading, and results stages; a centered preparation layout; equal search/filter actions; a centered, touch-blocking filter modal; staged filters with explicit **Aplicar filtros** confirmation and an active-filter counter; compact result status text and a non-technical source-coverage indicator; a direct Android `RecyclerView`/`ViewHolder` results surface with native inertial physics; compact adaptive cards; back and scroll-to-top actions; local document selection; theme-aware in-app filter choice panels; favorites; theme persistence; five institutional sections; and original vacancy links. New installations use **Últimas 24 horas** as the default publication window; the chosen filter remains local to each device. Curriculum analysis is identified separately from job searching and happens locally. The page and application architecture remain MAUI; only the high-volume results surface uses its Android-native handler to avoid MAUI cell measurement during a gesture. It embeds the same employer catalog used by desktop and keeps a 30-minute persistent cache. The Android cache is versioned when collection policy changes, so older partial entries are not reused by a newer mobile build. The Android first pass uses lightweight Greenhouse listings rather than complete vacancy HTML, while desktop retains complete descriptions. Active vacancies without a public publication date remain eligible and are visibly identified as such; vacancies with known dates still obey the selected period. Source collection, cache reading, normalization, matching, and cache writes run outside the visual path. All configured sources start in parallel, and the application presents one consolidated result set after collection finishes or reaches the overall deadline; it never shows an intermediate empty result as final. The mobile collector uses a 60-second overall budget, 40 seconds per source, HTTP requests up to 45 seconds and one retry for transient failures; five sources execute concurrently to avoid overloading the device network stack. SmartRecruiters evaluates up to eight controlled query variants per catalog company, prioritizing the Portuguese and English internship forms. Searches cancel on network loss and emit privacy-safe source timing diagnostics. See the [device testing guide](docs/ANDROID_TESTING.md).
